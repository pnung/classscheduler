...
public class EditTaskOnClickListener implements View.OnClickListener {
    ...
    public EditTaskOnClickListener(FragmentActivity requiredActivity, Context context, TaskViewModel taskViewModel, int position) {
    ...
    public void onClick(View v) {
        ...
        selectMonthSpinner.setSelection(currentTask.getPrimaryDateAndTime().getMonth() - 1);
        selectDaySpinner.setSelection(currentTask.getPrimaryDateAndTime().getDay() - 1);
        selectYearSpinner.setSelection(currentTask.getPrimaryDateAndTime().getYear() - 2024);
        selectHourSpinner.setSelection(currentTask.getPrimaryDateAndTime().getHour() % 12);
        selectMinuteSpinner.setSelection(currentTask.getPrimaryDateAndTime().getMinute() / 5);
        selectAmOrPmSpinner.setSelection(currentTask.getPrimaryDateAndTime().isPM() ? 1 : 0);
        ...
        selectMonthSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                dayArrayAdapter = new ArrayAdapter<>(context, android.R.layout.simple_spinner_dropdown_item, days);
                selectDaySpinner.setAdapter(dayArrayAdapter);
                if (currentTask.getPrimaryDateAndTime().getDay() >= days.length) {
                    currentTask.getPrimaryDateAndTime().setDay(days.length);
                }
                selectDaySpinner.setSelection(currentTask.getPrimaryDateAndTime().getDay() - 1);
            }
            @Override
